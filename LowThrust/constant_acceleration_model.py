...
for model_test in range(number_of_models):
    ...
    # For case 3, the ephemeris of Jupiter is generated by solving the 2-body problem of the Sun and Jupiter
...
for model_test in range(1, number_of_models):
    ...
    plt.plot(np.array(list(state_difference_wrt_nominal.keys())),
             np.linalg.norm(np.array(list(state_difference_wrt_nominal.values()))[:, 0:3], axis=1), label='norm',
             linestyle='dashed')
    plt.plot(np.array(list(state_difference_wrt_nominal.keys())),
             np.array(list(state_difference_wrt_nominal.values()))[:, 0], label='x')
    plt.plot(np.array(list(state_difference_wrt_nominal.keys())),
             np.array(list(state_difference_wrt_nominal.values()))[:, 1], label='y')
    plt.plot(np.array(list(state_difference_wrt_nominal.keys())),
